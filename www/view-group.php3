<?
	...
	if (pg_numrows($result) > 1)
		echo "Warning: more than one group entity with ID $entity_id was found<BR>\n";
	$row = pg_fetch_array ($result, 0);
	echo "<table width=100%><tr><td><font size=+1>$row[name]";
	$group_type = $row[type];
	$group_name = $row[name];
	echo "</font></td>";
	echo "<td align=right><a href=enter-group.php3?entity_id=$row[entity_id]&group_type=$group_type>edit </a>";
	echo "<a href=delete-group.php3?entity_id=$row[entity_id]>delete</a></td></table>";
	echo "<table width=100%>\n<tr bgcolor=lightyellow><th align=left width=50%>Location</th>\n";
	echo "<th  width=50% align=left>Address</th><th align=right> <a href=enter-location.php3?entity_id=$entity_id&is_group=true>new</a></th></tr>\n";
	$result = pg_Exec ($conn, "SELECT * from group_type WHERE type_id=$group_type");
	$row = pg_fetch_array ($result, 0);
	...
	if (pg_numrows($result) > 0)
	{
		for ($rc = 0; $rc < pg_numrows($result); ++$rc)
		{
			...
			echo "   <td align=right valign=top><a href=enter-location.php3?location_id=$row[location_id]&entity_id=$entity_id&is_group=true>edit</a> ";
			echo "<a href=delete-location.php3?location_id=$row[location_id]&entity_id=$entity_id&is_group=true>delete</a></td></tr>\n";
		}
	}
	echo "</table>\n";
	...
	echo "<th align=left width=25%>Description</th><th align=right><a href=enter-contact-method.php3?entity_id=$entity_id&is_group=1>new</a></th></tr>\n";
	if (pg_numrows($result) > 0)
	{
		for ($rc = 0; $rc < pg_numrows($result); ++$rc)
		{
			...
			echo "   <td align=right valign=top><a href=enter-contact-method.php3?entity_id=$entity_id&detail=$detail&is_group=1>edit</a> ";
			echo "<a href=delete-contact-method.php3?entity_id=$entity_id&detail=$detail&is_group=1>delete</a></td></tr>\n";
		}
	}
	echo "</table>\n";
	echo "<table width=100%>\n";
	...
	echo "<th align=right>";
	echo "<a href=enter-relationship.php3?entity_id=$entity_id&is_group=1&selected_type=$default_relationship&direction=rev>new</a></th></tr>\n";
	$is_group = 1;
	...
	echo "</td></tr>";
	echo "<tr><td colspan=3 align=right><hr><a href=mailmerge-setup.php3?entity_id=$entity_id&relationship_type=$relationship_type&group_name=";
	echo urlencode($group_name) . ">mail merge</a></td></tr>";
	echo "</table>\n";
	echo "<table width=100%>\n";
	echo "<tr bgcolor=lightyellow><th width=100% align=left>Other Relationships</th>\n";
	echo "<th align=right><a href=enter-relationship.php3?entity_id=$entity_id&is_group=1>new</a></th></tr>\n";
	echo "<tr><td colspan=2><form action=search-relationships.php3?is_group=1 method=post>\n";
    ...
    echo "<th align=right><a href=enter-entity-comment.php3?entity_id=$entity_id&is_group=true>new</a></th></tr>\n";
    if (pg_numrows($result) > 0)
    {
        for ($rc = 0; $rc < pg_numrows($result); ++$rc)
        {
            ...
            echo "   <td align=right valign=top><a href=enter-entity-comment.php3?comment_id=$row[comment_id]&is_group=true&entity_id=$entity_id>edit</a> ";
			echo "<a href=delete-entity-comment.php3?comment_id=$row[comment_id]&is_group=true&entity_id=$entity_id>delete</a></td></tr>\n";
        }
    }
    echo "</table>\n";
?>
