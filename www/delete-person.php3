<?
	...
	if (!$entity_id)
	{
		echo "No person ID given, cannot uniquely identify record";
		exit;
	}
	if (!$sure)
	{
        ...
        echo "<font color=red size=+1>$row[othernames] $row[lastname]</font> along with all associated records?</P>";
		echo "<a href = delete-person.php3?entity_id=$entity_id&sure=yes><img src = images/yes.png alt=YES></a>";
		echo "<a href = view-person.php3?entity_id=$entity_id><img src = images/no.png alt=NO></a>";
		echo "</center>";
		exit;
	}

	$result = pg_Exec ($conn, "DELETE FROM person WHERE entity_id=$entity_id");
	if (!$result) 	
	{
		echo "An error occured.\n";
	    exit;
