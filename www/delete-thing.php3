<?
	...
	if (!$sure)
	{
		...
		echo "<a href = delete-thing.php3?entity_id=$entity_id&sure=yes><img src = images/yes.png alt=YES></a>";
		echo "<a href = view-thing.php3?entity_id=$entity_id><img src = images/no.png alt=NO></a>";
		echo "</center>";
		exit;
	}

	$result = pg_Exec ($conn, "DELETE from thing WHERE entity_id=$entity_id");
	if (!$result) 	
	{
		echo "An error occured.\n";
	    exit;
